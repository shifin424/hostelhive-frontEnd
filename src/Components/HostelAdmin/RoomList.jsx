            ...
            <tbody>
              {rooms.map((room, index) => (
                <tr className={index % 2 === 0 ? 'bg-gray-200' : 'bg-gray-300'} key={room._id}>
                  ...
                  <td className={`p-3 font-semibold ${room.status === 'vacant' ? 'text-red-600' : room.status === 'occupied' ? 'text-green-600' : 'text-orange-600'}`}>
                    {room.status}
                  </td>
                  <td className="p-3 font-semibold ">
                    <Button className='outline-2 border-black' onClick={() => handleModalOpen(room.roomImage.url)}>View</Button>
                  </td>
